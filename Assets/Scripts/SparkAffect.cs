...
public class SparkAffect : MonoBehaviour
{
	...
	private readonly BoolReactiveProperty _sparkAttackRP = new BoolReactiveProperty();
	...
	private void Start()
	{
		this.UpdateAsObservable().Select(_ => _playerSystem.SparkAttack)
			.Subscribe(x => _sparkAttackRP.Value = x);

		_sparkAttackRP.TakeUntilDestroy(this).Subscribe(_sparkDamageArea.SetActive);
